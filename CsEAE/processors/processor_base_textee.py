...
class DSET_processor:
    ...
    def _create_example_rams(self, lines):
        ...
        examples = []
        for line in lines:
            if len(line["event_mentions"]) == 0:  # 没有事件的句子就跳过了
                continue
            doc_key = line["doc_id"]  # "doc_key"记录了事件提及的编号
            ...
            for event_idx, event in enumerate(events):
                ...
                # co-occur
                event_types = []
                for event_temp in line['co_events']:
                    event_type = {}
                    event_type['event_type'] = event_temp
                ...
                event_trigger = dict()
                event_trigger['start'] = event['trigger']['start']
                event_trigger['end'] = event['trigger']['end']
                # 提取trigger的txt文本
                ...
                for num, sent_start in enumerate(first_word_locs):
                    if event_trigger['start'] >= sent_start and event_trigger['end'] <= end_word_locs[num]:
                        assert num == event['trigger']['sent_idx']
                ...
                for arg_info in event['arguments']:
                    ...
                    evt_arg['start'] = arg_info['start']
                    evt_arg['end'] = arg_info['end']
                    evt_arg['text'] = " ".join(full_text[evt_arg['start']:evt_arg['end']])
                    assert evt_arg['text'] == arg_info['text']
                    evt_arg['role'] = arg_info['role']  # 提取论元对应的角色
                    event_args.append(evt_arg)

                if event_idx > 0:
                    examples.append(Event(doc_key + str(event_idx), None, cut_text, event_type, event_trigger,
                                          event_args, full_text, first_word_locs, sentences,
                                          event_types))
