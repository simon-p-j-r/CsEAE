...
def get_res_list(res_path, args, filter_no_prob=False, dataset_type=None, train=False):
    ...
    if dataset_type == 'MLEE':
        ...
        if args.overlap:
            for line_num, line in enumerate(lines):
                line_event_dic = {}
                line_event_dic['id'] = line[0]
                line_event_dic['context'] = line[2]
                # for i, sentence in enumerate(line_event_dic['sentences']):
                #     line_event_dic['sentences'][i] = ' '.join(sentence)
                line_event_dic['event_mentions'] = []
                line_event = {}
                ...
                line_event_dic['events'] = line[8]
                line_event_dic['sents'] = line[9]
                lines[line_num] = line_event_dic
        ...
        for line in lines:
            ...
            for i, event in enumerate(event_now):
                event_line = {}
                event_line['id'] = doc_key + '_event_{}'.format(i)
                event_info = {}
                event_info['occur_event'] = event_types
                ...
                assert trigger['sent_idx'] == event_trigger['sent_idx']
                event_info['trigger'] = trigger
                event_info['start'] = event_trigger['start']
                event_info['end'] = event_trigger['end']
                event_info['word'] = event_trigger['text']
